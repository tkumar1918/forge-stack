package dev.tushar.forgestack.api.repository;

import dev.tushar.forgestack.githubinstallation.AvailableRepository;
import dev.tushar.forgestack.githubinstallation.ManagedRepositoryService;
import dev.tushar.forgestack.githubinstallation.ManagedRepositoryView;
import dev.tushar.forgestack.githubinstallation.RepositorySyncReport;
import dev.tushar.forgestack.githubinstallation.RepositorySyncService;
...
/**
 * Choosing which repositories ForgeStack maintains.
 *
 * <p>Listing and enabling are separate operations on purpose: {@code GET} shows what the
 * installation exposes, and only {@code POST .../manage} asks ForgeStack to look after one. Nothing
...
class RepositoryController {
    ...
    /** Everything this workspace could have ForgeStack maintain, and whether it already does. */
    @GetMapping
    List<AvailableRepository> available(@AuthenticationPrincipal ForgeStackPrincipal principal) {
     ...
     * which repositories the App can see needs a way to make ForgeStack notice without reinstalling.
     */
    @PostMapping("/sync/{installationId}")
    ResponseEntity<RepositorySyncReport> resync(
            @PathVariable long installationId, @AuthenticationPrincipal ForgeStackPrincipal principal) {
        return ResponseEntity.ok(sync.sync(principal.activeWorkspaceId(), installationId));
    }

    /** Asks ForgeStack to start maintaining one repository. */
    @PostMapping("/{repositoryId}/manage")
    ResponseEntity<ManagedRepositoryView> manage(
            @PathVariable UUID repositoryId, @AuthenticationPrincipal ForgeStackPrincipal principal) {
    ...
    /** Stops ForgeStack maintaining it. The repository stays available to re-enable. */

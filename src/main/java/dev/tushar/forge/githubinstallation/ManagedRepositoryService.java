...
/**
 * Turning Forge's maintenance of a repository on and off.
 *
...
public class ManagedRepositoryService {
    ...
    public Optional<ManagedRepositoryView> enable(UUID workspaceId, UUID repositoryId, UUID actorId) {
        return tenantScope.runInTenant(workspaceId, () -> {
            ...
            if (repository.isEmpty() || repository.get().getRemovedAt() != null) {
                // Enabling something Forge cannot currently see would create a managed repository
            ...
            log.info(
                    "Forge now maintains {} at autonomy {}",
     ...
     *
     * <p>Exposes {@code status} so callers can distinguish a human pausing maintenance from Forge
